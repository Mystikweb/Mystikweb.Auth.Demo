...
internal sealed class TokenRefreshingDelegatingHandler(OpenIddictClientService service, HttpMessageHandler innerHandler)
    : DelegatingHandler(innerHandler)
{
    ...
    private static string GetBackchannelAccessToken(HttpRequestOptions options) =>
        options.TryGetValue<string>(new(Tokens.BackchannelAccessToken), out var token) ? token :
    ...
    private static DateTimeOffset? GetBackchannelAccessTokenExpirationDate(HttpRequestOptions options) =>
        options.TryGetValue<string>(new(Tokens.BackchannelAccessTokenExpirationDate), out var token) &&
    ...
    private static string GetRefreshToken(HttpRequestOptions options) =>
        options.TryGetValue<string>(new(Tokens.RefreshToken), out var token) ? token :
