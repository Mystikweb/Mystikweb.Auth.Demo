    ...
    UserManager<ApplicationUser> userManager) : Controller
{
    ...
    public async Task<IActionResult> Authorize()
    {
        ...
        // Updated code to handle potential null reference for 'request.ClientId'
        var clientId = request.ClientId;
        if (string.IsNullOrEmpty(clientId))
            throw new InvalidOperationException("The client ID cannot be null or empty.");

        // Retrieve the application details from the database.
        var application = await applicationManager.FindByClientIdAsync(clientId) ??
            throw new InvalidOperationException("Details concerning the calling client application cannot be found.");

        var applicationClientId = await applicationManager.GetIdAsync(application);
        ...
        switch (await applicationManager.GetConsentTypeAsync(application))
        {
            ...
            case ConsentTypes.Explicit when authorizations.Count is not 0 && !request.HasPromptValue(PromptValues.Consent):
                ...
                applicationClientId = await applicationManager.GetIdAsync(application);
                if (string.IsNullOrWhiteSpace(applicationClientId))
                    throw new InvalidOperationException("The Application ID cannot be null or empty.");
            ...
            default:
                ...
                {
                    ApplicationName = await applicationManager.GetLocalizedDisplayNameAsync(application) ?? throw new InvalidOperationException("Unable to determine the application name."),
                    Scope = request.Scope ?? throw new InvalidOperationException("The scope cannot be null or empty.")
                });
        }
    }
